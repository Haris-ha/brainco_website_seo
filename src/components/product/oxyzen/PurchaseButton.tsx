...
export default function PurchaseButton({ product, isMobile = false }: PurchaseButtonProps) {
  ...
  if (isMobile) {
    return (
      <motion.button
        ...
        disabled={isLoading}
        className="text-fluid-2xl h-[50px] w-full cursor-pointer rounded-[25px] bg-[#4f68d2] font-medium text-white transition-all hover:bg-[#3d52a8] disabled:cursor-not-allowed disabled:opacity-50"
      >
  ...
  return (
    <motion.button
      ...
      onClick={handleBuyNow}
      disabled={isLoading}
      className="cursor-target text-fluid-3xl mt-6 flex h-20 w-[260px] items-center justify-center rounded-[40px] bg-[#4f68d2] text-white shadow-lg transition-all hover:bg-[#3d52a8] disabled:cursor-not-allowed disabled:opacity-50"
    >
      {isLoading ? tCart('submitting') : t('buy_now')}
    </motion.button>
  );
