...
export default function PurchaseButton({ product, isMobile = false }: PurchaseButtonProps) {
  ...
  return (
    <motion.button
      ...
      onClick={handleBuyNow}
      disabled={isLoading}
      className="cursor-target text-fluid-3xl mt-6 flex h-[72px] w-[260px] items-center justify-center rounded-[40px] bg-[#1A74BF] !text-white shadow-lg transition-all hover:bg-[#155a96] disabled:cursor-not-allowed disabled:opacity-50"
    >
      {isLoading ? tCart('submitting') : t('buy_now')}
    </motion.button>
  );
