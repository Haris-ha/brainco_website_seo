...
export function StepPaymentMethod({
  ...
  return (
    ...
    >
      {/* 步骤指示 */}
      <nav aria-label={t('step_indicator', { current: 3, total: 3 })}>
      ...
      {/* 标题 */}
      <h1 id="step-3-title" className="text-3xl text-[#333] md:text-4xl lg:text-[56px]">
        {t('step_3_title')}
      </h1>
      ...
      {qrValue && (
        <section className="flex flex-col items-center" aria-labelledby="qr-code-title">
          <h2 id="qr-code-title" className="sr-only">{t('scan_to_pay', { method: paymentMethod === 'WECHAT_PAY_NATIVE' ? '微信' : '支付宝' })}</h2>
          ...
          <figure className="flex h-[180px] w-[180px] items-center justify-center md:h-[200px] md:w-[200px] lg:h-[240px] lg:w-[240px]">
            {paymentMethod === 'WECHAT_PAY_NATIVE' && (
              <div className="flex items-center justify-center bg-white p-3 md:p-4" role="img" aria-label="微信支付二维码 / WeChat Pay QR Code">
            ...
            {paymentMethod === 'ALIPAY_PAGE' && (
              <div id="payCode" className="flex items-center justify-center" role="img" aria-label="支付宝支付二维码 / Alipay QR Code">
                {/* 支付宝表单将在这里渲染 */}
                <iframe className="size-[180px] border-0 md:size-[200px] lg:size-[250px]" title="支付宝支付 / Alipay Payment" />
              </div>
            )}
          </figure>
