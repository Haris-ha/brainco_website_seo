  ...
  return (
    ...
    >
      ...
      <ul className="px-4 md:px-12">
        {(showMore ? items : items.slice(0, 2)).map(item => (
          ...
          >
            ...
            {/* 商品信息 */}
            <div className="flex flex-1 flex-col justify-around py-0.5 text-base text-[#595757] md:py-1 md:text-lg  lg:py-1.5 lg:text-xl">
              <div className="flex justify-between">
                <p className="mr-4 line-clamp-2 flex-1 overflow-hidden text-ellipsis md:mr-6 lg:mr-7.5">
                  {item.name}
                </p>
                <span className="flex-shrink-0 md:text-xl">
                  ¥
                  {item.price / 100}
                </span>
              </div>
      ...
      <div className="border-t border-dashed border-[#bebcbc] px-4 pt-3 pb-5 md:px-6 md:pt-4 md:pb-6 lg:px-8 lg:pt-5 lg:pb-7">
        {/* 折扣码 */}
        <div className="mb-1 flex justify-between text-base text-[#595757] md:text-[15px] lg:text-lg">
        ...
        {!discountAmount && currentStep !== 2 && (
          <div className="relative mt-1">
            <button
              type="button"
              className="cursor-target text-left text-base text-[#4F68D2] hover:underline md:text-lg lg:text-xl"
              onClick={(e) => {
                e.stopPropagation();
